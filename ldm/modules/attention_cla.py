...
class MemoryEfficientCrossAttention(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.0):
        ...
        # Adding the control kv projection layers
    ...
    def forward(self, x, context=None, control=None, mask=None, lambda_=1.0):
        ...
        # Control kv computation
        # Control kv computation
        if exists(mask) and exists(control):
            k_control = self.to_k_control(control)
            v_control = self.to_v_control(control)

            k_control, v_control = map(
                lambda t: t.unsqueeze(3)
                .reshape(b, t.shape[1], self.heads, self.dim_head)
                .permute(0, 2, 1, 3)
                .reshape(b * self.heads, t.shape[1], self.dim_head)
            ...
            # Use xformers for control attention
            out_control = xformers.ops.memory_efficient_attention(q, k_control, v_control, attn_bias=None, op=self.attention_op)
            out_control = rearrange(out_control, '(b h) n d -> b n (h d)', h=self.heads)

        # Compute the standard attention
        out = xformers.ops.memory_efficient_attention(q, k, v, attn_bias=None, op=self.attention_op)
        ...
        # Apply control flow output if mask exists
        if exists(mask) and exists(control):
...
#         if exists(mask):
#             control = default(control, context)
#             k_control = self.to_k_control(control)
#             v_control = self.to_v_control(control)

#             k_control = rearrange(k_control, 'b n (h d) -> b n h d', h=h)
#             v_control = rearrange(v_control, 'b n (h d) -> b n h d', h=h)

#             sim_control = xformers.ops.memory_efficient_attention(q, k_control, v_control, attn_bias=None, op=xformers.ops.MemoryEfficientAttentionFlashAttentionOp)
#             sim_control = rearrange(sim_control, 'b n h d -> b n (h d)')

#         sim = xformers.ops.memory_efficient_attention(q, k, v, attn_bias=None, op=xformers.ops.MemoryEfficientAttentionFlashAttentionOp)
#         sim = rearrange(sim, 'b n h d -> b n (h d)')

#         if exists(mask):
#             sim = sim + lambda_ * sim_control
